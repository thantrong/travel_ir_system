...
async def run_crawler():
    ...
    async with async_playwright() as p:
        ...
        for row in all_reviews:
            hid = str(row.get("source_hotel_id", "")).strip()
            if not hid:
                continue
            place_map[hid] = {
                "_id": hid,
                "source_hotel_id": hid,
                "source": str(row.get("source", "traveloka")).strip().lower() or "traveloka",
                "name": row.get("hotel_name", ""),
                "type": (row.get("place_types") or ["hotel"])[0],
                "types": row.get("place_types") or ["hotel"],
                "type_source": row.get("place_type_source", "fallback"),
                "location": row.get("location", ""),
                "rating": row.get("rating", ""),
            }
        all_places = list(place_map.values())

        # Final save
        final_reviews_file = RAW_DIR / "traveloka_raw_final_reviews.json"
        final_places_file = RAW_DIR / "traveloka_raw_final_places.json"
        with open(final_reviews_file, "w", encoding="utf-8") as f:
            json.dump(all_reviews, f, ensure_ascii=False, indent=2)
        with open(final_places_file, "w", encoding="utf-8") as f:
            json.dump(all_places, f, ensure_ascii=False, indent=2)

        logger.info(f"\n{'='*60}")
        logger.info(f"CRAWLER FINISHED!")
        logger.info(f"Total reviews: {len(all_reviews)}")
        logger.info(f"Total places: {len(all_places)}")
        logger.info(f"Saved reviews to: {final_reviews_file}")
        logger.info(f"Saved places to: {final_places_file}")
        logger.info(f"{'='*60}")
