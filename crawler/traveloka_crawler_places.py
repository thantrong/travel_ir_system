...
def extract_place_type_from_soup(soup, hotel_name=""):
    """Suy loại hình place từ metadata chính thức trước, fallback theo tên."""
    text = normalize_text_spaces(soup.get_text(" ", strip=True)).lower()
    title = normalize_text_spaces(soup.title.get_text(" ", strip=True) if soup.title else "").lower()
    name = normalize_text_spaces(hotel_name or "").lower()
    haystack = f"{title} {text} {name}"
    ...
    if not found:
        found = ["hotel"]
    return found, ("official" if len(found) > 0 else "fallback")
...
async def scrape_reviews_from_detail(page, hotel_name, city_name, city_code, detail_url, max_reviews):
    ...
    place_types, place_type_source = extract_place_type_from_soup(soup, hotel_name_extracted)
